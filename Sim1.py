...
if __name__ == "__main__":
    ...
    #     seed = run + 1
    #     # 1) 数据模拟
    #     x_true, y_obs = simulate_kitagawa(T, Q_true, R_true, x0, seed=seed) #x_true 是真实状态序列，y_obs 是观测序列 数据类型为 numpy.ndarray
    #     # 2) SIR-PF
    #     x_sir, ESS_sir, t_sir = SIR_PF(y_obs, N, Q_true, R_true, x0, prior_std)
    #     errors_sir[run, :] = (x_sir - x_true) ** 2
    #     rmse_sir.append(np.sqrt(np.mean((x_true - x_sir)**2)))
    #     time_sir.append(t_sir)
    #     ESS_sir_all[run, :] = ESS_sir
    #
    #     # 3) EKF-PF
    #     x_epf, ESS_epf, t_epf = EKF_PF(y_obs, N, Q_true, R_true, x0, P0=1.0)
    #     errors_epf[run, :] = (x_epf - x_true) ** 2
    #     rmse_epf.append(np.sqrt(np.mean((x_true - x_epf)**2)))
    #     time_epf.append(t_epf)
    #     ESS_epf_all[run, :] = ESS_epf
    #
    #     # # 4) UPF
    #     x_upf, ESS_upf, t_upf = UPF(y_obs, N, Q_true, R_true, x0, P0=1.0)
    #     errors_upf[run, :] = (x_upf - x_true) ** 2
    #     rmse_upf.append(np.sqrt(np.mean((x_true - x_upf)**2)))
    #     time_upf.append(t_upf)
    #     ESS_upf_all[run, :] = ESS_upf
    #
    #     # 5) BCPS‑PF
    #     x_bcps, K_list, t_bcps = BCPS_PF(y_obs, N, Q_true, R_true,
    #                                      x0, alpha=alpha, prior_std=prior_std,
    #                                      max_batches=max_batches)
    #     K_list_bcps.append(K_list)
    #     errors_bcps[run, :] = (x_bcps - x_true) ** 2
    #     rmse_bcps.append(np.sqrt(np.mean((x_true - x_bcps)**2)))
    #     time_bcps.append(t_bcps)
    #     #检查rmse_bcps中是否有nan，如果有输入一个标签
    #     # if np.isnan(rmse_bcps).any():
    #     #     print("标签：包含NaN值")
    #     #     print(rmse_bcps)
    #     #     print(x_true)
    #     #     print(x_bcps)
    #
    #     # 6) APF
    #     x_apf, ESS_apf, t_apf = APF(y_obs, N, Q_true, R_true, x0, prior_std)
    #     errors_apf[run, :] = (x_apf - x_true) ** 2
    #     rmse_apf.append(np.sqrt(np.mean((x_true - x_apf)**2)))
    #     time_apf.append(t_apf)
    #     ESS_apf_all[run, :] = ESS_apf
    #
    # # 统计平均性能指标
    # print("===== 100 次 Monte Carlo 仿真结果 (N=", N, ") =====")
    # print(f"SIR-PF    平均 RMSE: {np.mean(rmse_sir):.4f}   平均耗时: {np.mean(time_sir):.4f} s")
    # print(f"EKF-PF    平均 RMSE: {np.mean(rmse_epf):.4f}   平均耗时: {np.mean(time_epf):.4f} s")
    # print(f"UPF       平均 RMSE: {np.mean(rmse_upf):.4f}   平均耗时: {np.mean(time_upf):.4f} s")
    # print(f"BCPS-PF   平均 RMSE: {np.mean(rmse_bcps):.4f}   平均耗时: {np.mean(time_bcps):.4f} s")
    # print(f"APF       平均 RMSE: {np.mean(rmse_apf):.4f}   平均耗时: {np.mean(time_apf):.4f} s")
    #
    # # BCPS-PF 的批次数统计
    # K_list_bcps = np.array(K_list_bcps)
    # print("BCPS-PF 平均批次数:", np.mean(K_list_bcps, axis=0))
    #
    # rmse_bcps_curve = np.sqrt(np.mean(errors_bcps, axis=0))  # shape: (T,)
    # rmse_sir_curve = np.sqrt(np.mean(errors_sir, axis=0))  # shape: (T,)
    # rmse_epf_curve = np.sqrt(np.mean(errors_epf, axis=0))  # shape: (T,)
    # rmse_upf_curve = np.sqrt(np.mean(errors_upf, axis=0))  # shape: (T,)
    # rmse_apf_curve = np.sqrt(np.mean(errors_apf, axis=0))  # shape: (T,)
    #
    #
    # # 平均 ESS 曲线
    # mean_ESS_sir = ESS_sir_all.mean(axis=0)
    # mean_ESS_epf = ESS_epf_all.mean(axis=0)
    # mean_ESS_upf = ESS_upf_all.mean(axis=0)
    # mean_ESS_apf = ESS_apf_all.mean(axis=0)
    #
    # # ======================
    # # 绘图：每个时间步的 RMSE 曲线
    # # plt.plot(rmse_bcps_curve, label='BCPS-PF')
    # # plt.plot(rmse_sir_curve, label='SIR-PF')
    # # plt.xlabel('Time step t')
    # # plt.ylabel('RMSE')
    # # plt.legend()
    # # plt.title('Per-step RMSE over MC simulations')
    # # plt.grid(True)
    # # plt.show()
    # t = np.arange(1, T + 1)
    #
    ...
    # # 绘制两条曲线
    #
    # plt.plot(t, rmse_sir_curve, label='SIR-PF', color='orangered', linewidth=1.0, marker='s', markersize=3)
    # plt.plot(t, rmse_apf_curve, label='APF', color='darkorange', linewidth=1.0, marker='x', markersize=3)
    # plt.plot(t, rmse_epf_curve, label='EKF-PF', color='forestgreen', linewidth=1.0, marker='^', markersize=3)
    # plt.plot(t, rmse_upf_curve, label='UPF', color='darkcyan', linewidth=1.0, marker='d', markersize=3)
    # plt.plot(t, rmse_bcps_curve, label='BCPS-PF', color='royalblue', linewidth=1.0, marker='o', markersize=3)
    #
    # # 设置坐标轴标签
    # plt.xlabel('Time Step $t$', fontsize=13)
    # plt.ylabel('RMSE', fontsize=13)
    ...
    # # 调整边距
    # plt.tight_layout()
    # # 保存高质量图像（推荐 PDF 用于论文插图）
    # plt.savefig('rmse_per_step_plot.pdf', format='pdf', bbox_inches='tight')
    # # 显示图像
    # plt.show()
    #
    # # 绘图：平均 RMSE 对比
    # plt.figure(figsize=(8,5))
    # # plt.bar(['SIR','EKF-PF','UPF','BCPS'],
    # #         [np.mean(rmse_sir), np.mean(rmse_epf), np.mean(rmse_upf), np.mean(rmse_bcps)],
    # #         color=['r','g','c','b'])
    # plt.bar(['SIR','BCPS'],
    #         [np.mean(rmse_sir), np.mean(rmse_bcps)],
    #         color=['r','g','c','b'])
    # plt.ylabel("Average RMSE over 100 runs")
    # plt.title("1D Kitagawa: 4 Algorithms Comparison")
    # plt.grid(axis='y', linestyle='--', alpha=0.6)
    # plt.tight_layout()
    # plt.show()
    #
    # # 绘图：平均 ESS 比较 (只对有权重的方法)
    # plt.figure(figsize=(8,5))
    # plt.plot(mean_ESS_sir, 'r-',  label='ESS (SIR-PF)')
    # # plt.plot(mean_ESS_epf, 'g--', label='ESS (EKF-PF)')
    # # plt.plot(mean_ESS_upf, 'c-.', label='ESS (UPF)')
    # plt.hlines(N/2, 0, T-1, colors='gray', linestyles='dashed', label='N/2 Threshold')
    # plt.xlabel("Time Step")
    # plt.ylabel("Mean ESS (100 runs)")
    # plt.title("Average ESS over Time")
    # plt.legend()
    # plt.grid(True)
    # plt.tight_layout()
    # plt.show()
